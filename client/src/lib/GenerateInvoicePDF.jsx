...
// Generate and return a Data URL while still triggering a download
export async function generateInvoicePDFAndGetDataUrl(data) {
  const { billItems, totalAmount, customerPhone } = data;
  if (!billItems || billItems.length === 0) {
    throw new Error("No items to generate invoice");
  }
  ...
  billItems.forEach((item) => {
    ...
    doc.text(item.name, 20, yPosition);
    doc.text(String(item.quantity), 80, yPosition);
    doc.text(`₹${Number(item.rate).toFixed(2)}`, 120, yPosition);
    doc.text(`₹${Number(item.amount).toFixed(2)}`, 160, yPosition);
  });
  ...
  // Trigger download
  doc.save(fileName);
  // Also return data URL for upload to server
